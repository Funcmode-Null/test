...
--// ADDITIONAL TAB: Auto Buy (Patched, Optimized & UX Improved)
AddTab("AutoBuy", function(parent)
    ...
    -- Persistent state for selections and delays
    local SaveKey = "TCS_AutoBuy_State"
    local function SaveAutoBuyState()
        local state = {
            Selected = selectedPerType,
            Delays = autoDelay
        }
        pcall(function()
            player:SetAttribute(SaveKey, HttpService:JSONEncode(state))
        ...
        if attr then
            ...
            if suc then
                if data.Selected then
                    selectedPerType.Plant = data.Selected.Plant or {}
                    selectedPerType.Gear = data.Selected.Gear or {}
                end
                if data.Delays then
                    autoDelay.Plant = data.Delays.Plant or 0.3
                    autoDelay.Gear = data.Delays.Gear or 0.3
                end
                return true
            end
        end
    ...
    -- Frame container (adjusted height for status label)
    local frame = New("Frame", {
        Parent = parent,
        Size = UDim2.new(1, -10, 0, 470),
    ...
    local function updateStatus()
        local plantRunning = runningLoops.Plant
        local gearRunning = runningLoops.Gear
        if plantRunning and gearRunning then
            statusLabel.Text = "Status: Buying Plants & Gears"
            statusLabel.TextColor3 = Theme.ColorVariants.success
        elseif plantRunning then
            statusLabel.Text = "Status: Buying Plants"
            statusLabel.TextColor3 = Theme.ColorVariants.success
        elseif gearRunning then
            statusLabel.Text = "Status: Buying Gears"
            statusLabel.TextColor3 = Theme.ColorVariants.success
        else
    ...
    local plantBtn = New("TextButton", {
        Parent = typeSelector, Size = UDim2.new(0.5,-5,1,0),
        Text = "Plant", Font = Theme.Font, TextSize = 14,
        BackgroundColor3 = Theme.Colors.Button, TextColor3 = Theme.Colors.Text,
    })
    local gearBtn = New("TextButton", {
        Parent = typeSelector, Position = UDim2.new(0.5,5,0,0), Size = UDim2.new(0.5,-5,1,0),
        Text = "Gear", Font = Theme.Font, TextSize = 14,
        BackgroundColor3 = Theme.Colors.Button, TextColor3 = Theme.Colors.Text,
    })
    ...
    local delayBox = New("TextBox", {
        ...
        TextColor3 = Theme.Colors.Text, BackgroundColor3 = Theme.Colors.Button,
        BorderSizePixel = 0, Text = tostring(autoDelay[selectedType]),
    })
    ...
    local btnContainer = New("Frame", {
        Parent = frame, Position = UDim2.new(0,10,0,155),
        Size = UDim2.new(1,-20,0,28), BackgroundTransparency = 1,
    })
    local clearBtn = New("TextButton", {
        Parent = btnContainer, Size = UDim2.new(0.33,0,1,0),
        Text = "Clear Selection", Font = Theme.Font, TextSize = Theme.TextSize,
        BackgroundColor3 = Theme.ColorVariants.error, TextColor3 = Theme.Colors.Text,
    })
    local selectAllBtn = New("TextButton", {
        Parent = btnContainer, Position = UDim2.new(0.33,4,0,0), Size = UDim2.new(0.33,0,1,0),
        Text = "Select All", Font = Theme.Font, TextSize = Theme.TextSize,
        BackgroundColor3 = Theme.ColorVariants.success, TextColor3 = Theme.Colors.Text,
    })
    local selectBothBtn = New("TextButton", {
        Parent = btnContainer, Position = UDim2.new(0.66,4,0,0), Size = UDim2.new(0.33,0,1,0),
        Text = "Select Both", Font = Theme.Font, TextSize = Theme.TextSize,
        BackgroundColor3 = Theme.Colors.Button, TextColor3 = Theme.Colors.Text,
    })
    ...
    -- Dropdown list (shifted position)
    local dropdownList = New("ScrollingFrame", {
        Parent = frame,
        Size = UDim2.new(1, -20, 0, 150),
        Position = UDim2.new(0,10,0,190),
        BackgroundColor3 = Theme.Colors.PanelAlt,
        ClipsDescendants = true, ScrollBarThickness = 6,
        AutomaticCanvasSize = Enum.AutomaticSize.None,
        CanvasSize = UDim2.new(0, 0, 0, 0),
    })
    local listLayout = New("UIListLayout", {Parent = dropdownList, Padding = UDim.new(0,4), SortOrder = Enum.SortOrder.LayoutOrder})
    ...
    local function getDropdownButton()
        ...
        if not btn then
            btn = New("TextButton", {
                ...
                Font = Theme.Font, TextSize = Theme.TextSize,
                TextColor3 = Theme.Colors.Text, BorderSizePixel = 0,
            })
            New("UICorner",{Parent=btn, CornerRadius=UDim.new(0,6)})
        else
    ...
    -- Keyboard shortcuts (guarded by frame visibility)
    ...
    -- Auto Buy toggle button (adjusted position)
    local autoBtn = New("TextButton", {
        Parent = frame,
        Size = UDim2.new(1,-20,0,36),
        Position = UDim2.new(0,10,0,350),
        Text = "Auto Buy: OFF", Font = Theme.FontBold, TextSize = 16,
        BackgroundColor3 = Theme.ColorVariants.error, TextColor3 = Theme.Colors.Text,
        BorderSizePixel = 0,
    })
    ...
    local bothAutoBtn = New("TextButton", {
        ...
        Text = "Auto Buy Both: OFF", Font = Theme.FontBold, TextSize = 16,
        BackgroundColor3 = Theme.ColorVariants.error, TextColor3 = Theme.Colors.Text,
        BorderSizePixel = 0,
    })
    ...
    cleanupConn = frame.AncestryChanged:Connect(function()
        if not frame.Parent then
            ...
            SaveAutoBuyState()  -- Final save on destroy
        end
    end)
